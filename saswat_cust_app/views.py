...
from saswat_cust_app.models import (UserOtp, UserDetails, CustomerTest, Gender, State, VleVillageInfo,
                                    ...
                                    AutopayAssigned, CollectionAutopay, EMIDetails)
...
class CollectionDataView(APIView):
        ...
        try:
            ...
            if not employee_details:
                return self._generate_failure_response(
                    "Employee details not found for the user"
                )

            designation_name = DesignationDetails.objects.get(
                id=employee_details.designation_id
            ).designation_name
    ...
    def handle_selected_so(self, selected_so_id, loan_id, response_data):
        try:
            ...
            return self._generate_failure_response(
                "Loan Applications not found for the Sales Officer"
            )
    ...
    def handle_selected_rm(self, selected_rm_id, loan_id, response_data):
        try:
            ...
            rm_designation_name = DesignationDetails.objects.get(
                id=selected_rm_data['designation']
            ).designation_name
            selected_rm_data['designation_name'] = rm_designation_name

            for so in sales_officer_data:
                so['designation_name'] = DesignationDetails.objects.get(
                    id=so['designation']
                ).designation_name
    ...
    def handle_cluster_head(self, employee_details, response_data, loan_id):
        try:
            ...
            reporting_managers = EmployeeDetails.objects.filter(
                cluster_head=employee_details,
                designation=rm_designation.id
            )
            sales_officers = EmployeeDetails.objects.filter(
                cluster_head=employee_details,
                designation=so_designation.id
            )
            direct_sales_officers = sales_officers.filter(reporting_manager=None)

            response_data["reporting_manager"] = EmployeeDetailsSerializer(
                reporting_managers, many=True
            ).data
            response_data["sales_officer"] = EmployeeDetailsSerializer(
                direct_sales_officers, many=True
            ).data
    ...
    def handle_sales_officer(self, employee_details, response_data, loan_id):
        try:
            ...
            return self._generate_failure_response(
                "Collection is not found for the Sales Officer"
            )

    def get_emi_collections(self, loan_id):
        emi_collections = EMICollections.objects.filter(id=loan_id)
        emi_detail = EMIDetails.objects.filter(
            lender_loan_id=loan_id,
            collections_status__in=[
                "Collected - Need to be Verified",
                "Partial collected - Need to be verified",
                "Not-Collected",
            ],
        ).order_by('-created_at')

        collection_paid = CollectionPayment.objects.filter(
            loan_id__loan_details__lender_loan_id=emi_coll.lender_loan_id).order_by('-created_at').first()

        if collection_paid:
            last_paid_details = [{
                'paid_amount': collection_paid.paid_amount,
                'paid_date': collection_paid.created_at,
                'status': collection_paid.status
            }]
        else:
            last_paid_details = []
        ...
        if emi_collections.exists():
            emi_serializer = EMICollectionsSerializer(emi_collections, many=True)
            emi_details_serializer = EMIDetailsSerializer(emi_detail, many=True)

            emi_details_mapping = {}
            for emi_detail in emi_details_serializer.data:
                lender_loan_id = emi_detail.get('lender_loan_id')
                if lender_loan_id not in emi_details_mapping:
                    emi_details_mapping[lender_loan_id] = []
                emi_details_mapping[lender_loan_id].append(emi_detail)

            for emi_collection in emi_serializer.data:
                lender_loan_id = emi_collection.get('lender_loan_id')
                emi_collection['emi_details'] = emi_details_mapping.get(lender_loan_id, [])

            total_balance_amt = Decimal('0.00')
            for emi in emi_details_serializer.data:
                balance_amt = Decimal(emi.get('balance_amt', '0.00'))
            ...
            response_data = {
                ...
                "total_balance_amount": str(total_balance_amt),
                "last_paid_details": last_paid_details,
            }
            return Response(response_data)
        else:
    ...
    def post(self, request, *args, **kwargs):
        ...
        collections_to_update = EMIDetails.objects.filter(
            lender_loan_id__lender_loan_id=lender_loan_id
        ).order_by('created_at')

        if serializer.is_valid():
            validated_data = serializer.validated_data

            excess_amount = Decimal(str(paid_amt))

            for emi_details in collections_to_update:
                if excess_amount <= 0:
                    break

                amount_to_apply = min(excess_amount, emi_details.balance_amt)
                emi_details.paid_amt += amount_to_apply
                emi_details.balance_amt -= amount_to_apply
                excess_amount -= amount_to_apply
                emi_details.balance_amt = max(emi_details.balance_amt, 0)

                emi_details.payment_status = pay_status
                emi_details.collections_status = collection_status
                emi_details.save()

            if excess_amount > 0:
                raise ValidationError(
                    "The total paid amount exceeds the total EMI amount. Excess amount remaining: {}".format(
                        excess_amount)
                )
            ...
            validated_data['balance_amount'] = emi_details.balance_amt
            validated_data['emi_amount'] = emi_details.emi_amt

            collection_payment = serializer.save(
                balance_amount=validated_data['balance_amount'],
                emi_amount=validated_data['emi_amount']
            )
    ...
    def get_collection_status(self, pay_status):
        status_mapping = {
            "Paid": "Collected - Need to be Verified",
            "Paid-Partially": "Partial collected - Need to be verified",
            "Not Paid": "Not collected",
            "CASH-Paid": "Collected - Need to be Verified",
            "CASH-Paid-Partially": "Partial collected - Need to be verified",
            "Unable to Pay": "Not collected",
            "Applicant-Unavailable": "Not collected",
            "Promise-To-pay": "Not collected"
        }
...
class AutopayRegisterAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = {
            ...
            'udf1': request.data.get('lender_loan_id'),
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(response_data, dict):
                    ...
                    error_desc = response_data.get('error_desc')
                    if status_value == 0:
                        return self._generate_failure_response("{}".format({"error_desc":error_desc}))
